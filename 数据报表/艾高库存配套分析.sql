...
,c.FNumber stock,null qlsl,null fitemid,null wlxqsl,null mqlsl
into #sub
from ICBOM a 
...
,case when p.ptsl=0 then 0 else case when isnull(b.FAuxQty,0)*isnull(p.ptsl,0)-(isnull(j.FQty,0)-isnull(g.FWillOutQty,0))<0 then 0 else isnull(b.FAuxQty,0)*p.ptsl-(isnull(j.FQty,0)-isnull(g.FWillOutQty,0)) end end mqlsl
from ICBOM a 
left join ICBOMCHILD b on a.FInterID=b.FInterID
left join t_ICItem c on a.FItemID=c.FItemID 
...
left join (select FItemID,sum(FQty)FQty from ICInventory where FStockID in(279,281,284,285,286,4233,4452)  group by FItemID) j on d.FItemID=j.FItemID  
...
-- order by wlid
 ...
 sum(isnull(a.dwrl,0)) 单位用量, isnull(a.jskcsl,0) 即时库存数量,isnull(c.FWillOutQty,0)已分配量,
 ...
 isnull(a.jskcsl,0) ,isnull(c.FWillOutQty,0),
 ...
 , isnull(convert(varchar(10),b.ycdhrq ,120),''),a.fno
order by a.fno



----   KCFX '','','','','','','','','01.020102206',100,'','','01.010201802',20,'01.010100801',40,''

select
 物料长代码,物料,规格型号,计量单位,
 单位用量,即时库存数量,物料需求数量,
 case when (物料需求数量-即时库存数量)<0  then 0 else case when fno=1 then 物料需求数量-即时库存数量 else 缺料数量 end end 缺料数量
,未到物料数量,订单日期,订货数量 ,交货日期,延长到货日期,入库数量,入库日期,已分配量 ,[缺料数量(毛需求)]
 from  #lresult a
